...
public class VistaTexto extends Vista {
	...
	private Map<TipoVehiculo, Integer> inicializarEstadisticas() {
		...
		Map<TipoVehiculo, Integer> mapaEstadisticas = new HashMap<>();

		for (Map.Entry<TipoVehiculo, Integer> estadisticasMensuales : mapaEstadisticas.entrySet()) {
			mapaEstadisticas.put(estadisticasMensuales.getKey(), estadisticasMensuales.getValue());
	...
	public void mostrarEstadisticasMensualesTipoVehiculo() {
		...
		try {
			...
			if (!mapaEstadisticas.isEmpty()) {
				for (Alquiler alquiler : alquileres) {
					if (alquiler.getFechaAlquiler().getMonth().equals(mes.getMonth())
						...
						if (TipoVehiculo.get(alquiler.getVehiculo()).equals(TipoVehiculo.TURISMO)) {
							mapaEstadisticas.put(TipoVehiculo.TURISMO,
									mapaEstadisticas.getOrDefault(TipoVehiculo.AUTOBUS, 0) + 1);
						}

						if (TipoVehiculo.get(alquiler.getVehiculo()).equals(TipoVehiculo.FURGONETA)) {
							mapaEstadisticas.put(TipoVehiculo.FURGONETA,
									mapaEstadisticas.getOrDefault(TipoVehiculo.FURGONETA, 0) + 1);
						}

						if (TipoVehiculo.get(alquiler.getVehiculo()).equals(TipoVehiculo.AUTOBUS)) {
							mapaEstadisticas.put(TipoVehiculo.AUTOBUS,
									mapaEstadisticas.getOrDefault(TipoVehiculo.AUTOBUS, 0) + 1);
						}
				...
				Consola.mostrarCabecera("No hay estadisticas que mostrar en este mes.");
